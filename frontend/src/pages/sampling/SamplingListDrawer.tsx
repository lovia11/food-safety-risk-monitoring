    ...
    >
      {historical && (
        <div className="readonly-banner">
          ...
          <div>
            ...
            <p>以导出时 JSON 为业务事实，不会换成当前商品数据。</p>
          </div>
        </div>
      )}
      ...
      <section className="detail-section sampling-detail-facts">
        ...
        <dl>
          ...
          <div><dt>Snapshot ID</dt><dd>{item.sourceSnapshotId}</dd></div>
        ...
        <div className="detail-inline-actions">
          ...
          {!historical && <a className="text-link" href={`#/products/${encodeURIComponent(item.productId)}`}>查看商品快照</a>}
        </div>
      </section>
      ...
      <section className="detail-section">
        ...
        <div className="sampling-summary-grid">
          <div><span>可能风险方向</span><strong>{item.summary.riskDirections.join("、") || "暂无已保存线索"}</strong></div>
        ...
        <div className="sampling-method-list">
          ...
          )) : <p className="table-empty">暂无已核验方法。</p>}
        </div>
        {item.summary.riskDirections.length > 0 && item.summary.substances.length === 0 && (
      ...
      {(item.productContext.product_category ||
        ...
        <section className="detail-section sampling-detail-facts">
          <h3><Info size={16} />Product Context</h3>
          <dl>
      ...
      <section className="detail-section">
        ...
        {item.summary.evidenceQualifications.length > 0 && (
          <p className="qualification-note">
            Evidence qualification：{item.summary.evidenceQualifications.map(evidenceQualificationLabel).join("、")}
          </p>
        )}
