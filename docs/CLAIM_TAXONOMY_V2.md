...
| SQLite schema 13 (Claim tables introduced in 11) | Legacy Effect fields plus `claim_mentions`, `claim_signals`, `claim_signal_mentions` and per-Snapshot Claim artifact status/path | Rebuildable query projections. | Claim tables are additive; legacy fields and human state remain. |
