...
- The current Wide Reference Index has seven methods and is not a nationally complete method index. Six form the Deep Verified Subset. BJS 202405 is `recommendation_ready` only for its 95 explicit official-full-text analyte relations and seven parsed source scopes; revoked GB/T 5009.170-2003 remains `reference_only`. The Recommendation resolver admits only current `recommendation_ready` methods.
