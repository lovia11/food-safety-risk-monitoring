...
> Status: B5-1 ACCEPTED；B5-2 KJ201901/KJ201902 PROMOTED；B5-3A VERIFIED；B5-3B BJS201808 PROMOTED / ACCEPTED；B5-4A/B BJS202504 DEFERRED；B5-5A/B BJS202501 DEFERRED；B5-6A/B BJS202601 DEFERRED / RISK EVIDENCE BACKLOGGED；B5-7A/B BJS202602 DEFERRED；B5-8A/B BJS201901 PROMOTED / ACCEPTED；B5-9A/B BJS202409 DEFERRED / RUNTIME UNCHANGED；NEXT = B5-10A BJS202502 deep verification  
> Date: 2026-09-21  
...
`config/inspection_method_candidates_v2.json` 当前版本：`2026.09-b17`。
...
B5-5B 已确认 `verified_reference` validator 禁止 orphan Substance；在官方 SAMR 正文/附件仍无法稳定恢复时，不允许仅凭第三方镜像把3个 Substance + MethodSubstance 写入 runtime。
...
verification / reference_only
runtime unchanged
deferred pending official attachment
```

Next = **B5-6A BJS202601 deep verification**.  
Do not create concrete Risk relations as a side effect.
...
SAMR官方新闻中的“减肥、降压食品中违规掺入利尿降压类药物”进入独立 **Risk-governance backlog**，但不作为 Method→Risk side effect，不在B5直接写入Risk。

Next = **B5-7A BJS202602 deep verification**.

Canonical audit: `docs/V2_B5_BJS_202601_ANALYTE_AUDIT.md`.

## 5.5 B5-7A BJS 202602 audit result
...
official Method page transport = success=false / data={}
```
...
```text
UPLC-MS/MS
压片糖果 / 咖啡 / 苹果汁饮料 / 白酒 / 胶基糖果 / 果冻
```

但不作为 runtime authority。

Chemical identity 仍不满足治理 Gate：

- 多个商业来源把目标写作 O-Propyl Vardenafil / 2840532-32-1；
- PubChem 对该 CAS 使用不同 impurity synonym；
- 至少一个商业来源把完全不同 CAS/结构也标成 Vardenafil Impurity 30；
- 官方 O-丙基伐地那非标准样品项目尚未在可见信息中明确等同于 BJS202602 target。

当前 `male_function` 已有 `那非类、拉非类物质` current group mapping，但 `substance_group_memberships = 0`，不自动展开。
...
Next = **B5-10A BJS202502 deep verification**. BJS202409已在B5-9B明确deferred，不因第三方PDF存在而promotion。
