...
这两条并非按语义近似自由扩展。项目的受治理 HealthFunction 数据已经记录：
...
- `改善睡眠`：历史抽检资料直接使用该功能类别；
- `有助于改善睡眠`：通过已核验的官方功能名称迁移关系承接同一历史功能主题。
...
原因不是这些表达“没有意义”，而是当前 production Bridge 采用 exact-expression governance。不得因为同属 `sleep_related` 就整体外推 Risk。
...
对于仅命中“助眠”等尚未单独治理表达的页面，当前仍允许显示未映射；这是显式知识边界，不是旧链路 fallback。
